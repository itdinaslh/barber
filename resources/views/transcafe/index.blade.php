...
<div class="row">
    ...
    <div class="col-xs-12">
        <img src="{{ asset('/img/logo-cafe.png') }}" alt="Logo-Cafe" style="margin-left:auto;margin-right:auto;width:450px;" class="img img-responsive" />
    </div>
        ...
        <form id="FormAddSubTrans" class="form-inline" action="/transaction/cafe/addsubtrans" method="post">
            ...
            <div class="form-group">
                ...
                <select id="ProdList" class="form-control" name="idprod" required>
                    @foreach($prod as $v)
                        <option value="{{ $v->id }}" data-type="{{ $v->ProductType }}">{{ $v->ProductID }} - {{ $v->Nama }}</option>
                    @endforeach
                </select>
              </div>
              <div class="form-group">
              ...
              <div id="pricehid" class="form-group" style="display:none;">
                  <label for="">Harga</label>
                  <input type="number" id="HargaTemp" name="HargaTemp" class="form-control" value="" />
              </div>
              <div class="form-group">
    ...
    <div class="col-xs-12" style="margin-top:50px;">
        <div class="pull-right">
            <div class="pull-right form-inline">
                <div class="form-group">
                    <a href="{{ url('/transaction/cafe/print') }}" target="_blank" class="btn btn-warning">Print Struk Terakhir</a>
                </div>
                <div class="form-group">
                    <form id="FormClearCart" action="{{ url('/transaction/cafe/clearcart') }}" method="post">
                        {{ csrf_field() }}
                        <button type="submit" id="btnClearTrans" name="btnClearTrans" class="btn btn-danger">Cancel Transaksi</button>
                    </form>
                </div>
                <div class="form-group">
                    <button type="button" id="btnCheckOut" name="btnCheckOut" class="btn btn-info" data-href="{{ url('/transaction/cafe/getcheckout') }}">Bayar</button>
                </div>
            </div>
        </div>
    </div>
