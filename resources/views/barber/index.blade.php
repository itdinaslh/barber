                        ...
                        <tbody>
                            @foreach ($bbs as $v)
                                <tr>
                                    ...
                                    <td id="nama-{{ $v->id }}" data-nama="{{ $v->Nama }}">{{ $v->Nama }}
                                    </td>
                                    <td>{{ $v->IdNum }}</td>
                                    <td>{{ $v->Phone }}</td>
                                    @if ($v->Level == 'S')
                                    ...
                                    <td>
                                        <button type="button" name="btnEdit" class="btn btn-xs btn-success showMe"
                                            style="width:100%"
                                            data-href="/barberman/edit-{{ $v->id }}">Edit</button>
                                        <button type="button" name="btnService" class="btn btn-xs btn-warning sDetail"
                                            data-value="{{ $v->id }}"
                                            style="width:100%;margin-top:3px;">Service</button>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
        ...
        <div class="col-xs-12" style="margin-bottom:15px;">
            <div class="text-right">
                <button type="button" name="btnAdd" class="btn btn-primary showMe"
                    data-href="{{ url('/barberman/add') }}">Add New</button>
            </div>
        </div>
                ...
                <div class="box-body">
                    <form id="formService" class="form-inline" action="/barberman/addservice" method="post">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="fid" class="form-control" name="Fee" autocomplete="off"
                                value="0" required />
                        </div>
